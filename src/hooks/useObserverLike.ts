...
export default function useObserverLike(videoId?: string): [number?, FirebaseDatabaseTypes.Query?] {
	const [like, setLike] = useState(0)
	const ref = useMemo(
		() => (videoId ? database().ref(`like/${videoId}`).orderByChild('type').equalTo(1) : undefined),
		[videoId]
	)
